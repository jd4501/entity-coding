...
| Annotation IAA (kappa 0.81, exact-span F1 0.77)                                                                                                                                                                   | `ner/inter_annotator_agreement/`                                             | IAA inputs are project-internal and not committed to this repository. Scoring script and expected input layout are documented in [`ner/inter_annotator_agreement/README.md`](../ner/inter_annotator_agreement/README.md). |
...
**A note on the Section 5.5 manual analyses.** Both pieces (the 200-sample false-positive relevance categorisation and the qualitative subdivision of TP "No Overlap" cases) are manual reviews; no labels are committed. The underlying No Overlap pairs are derivable from `results/{entityonly,fulltext}_code_evidence_compared.csv` by re-running cell 9 of `code_evidence/code_evidence_eval.ipynb` after the [MDACE evidence flow](evidence.md). Those two CSVs are not shipped because their evidence-text columns contain snippets derived from gated MIMIC notes; running the notebook regenerates them in place. The qualitative labels themselves are not stored.
